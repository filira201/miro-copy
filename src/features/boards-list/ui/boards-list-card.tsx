...
export function BoardsListCard({
  ...
  return (
    <Card className="relative overflow-hidden h-full flex flex-col">
      {rightTopActions && (
        <div className="absolute top-2 right-2 z-10 shrink-0">
          {rightTopActions}
        </div>
      )}
      <CardHeader className="flex-1 min-w-0">
        <div className="flex flex-col gap-2 min-w-0 pr-8">
          <Button
            asChild
            variant="link"
            className="text-left justify-start h-auto p-0 min-w-0"
          >
            <Link
      ...
      {bottomActions && (
        <CardFooter className="shrink-0 mt-auto">{bottomActions}</CardFooter>
      )}
    </Card>
  );
}
